...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Registrasi Assets "<?php echo $account->first_name." ".$account->last_name ?>"
        </h1>
        <ol class="breadcrumb">
                        ...
                        <div class="box-body">
                            <div style="height:460px;">
                                <div style="background-color:#fcfdff; position: absolute center; height: 100%; width: 100%; margin:0 auto; overflow-y: scroll; box-shadow:0.5px 0.5px 2.5px 0.5px #8494b2 inset; border-radius: 5px 0 0 5px;">
                                    ...
                                    <div>
                                        <h5 class="box-title">
                                            <p><img class="profile-user-img img-responsive img-circle" src="<?php echo base_url('account/profile/image/'.$account->id) ?>" alt="<?php echo $account->first_name." ".$account->last_name ?>"></p>
                                        </h5>
                                    </div>
                                    <div>
                                        <h5 class="box-title">
                                    ...
                                    <form role="form" method="post" action="<?php echo base_url('member/assets/save')?>">
                                        ...
                                        <hr/>
                                        <?php foreach($list_document as $row): ?>
                                            <input name="assetsId<?php echo $row->legal_id ?>" value="<?php echo $row->assets_id ?>" type="hidden">
                                        <?php endforeach; ?>
                                        <input name="assetsId" value="" type="hidden">
                                        <input name="assetsId" value="" type="hidden">
                                        <input name="accountId" value="<?php echo $this->uri->segment(4) ?>" type="hidden">
                                        ...
                                        <div class="form-group">
                                            <input class="form-control" name="assetsName" placeholder="Nama Tempat" type="text" required>
                                            <span class="help-block with-errors"></span>
                                        </div>
                                        <div class="form-group">
                                            <textarea class="form-control" name="assetsAddress" placeholder="Alamat Lokasi"></textarea>
                                            <span class="help-block with-errors"></span>
                                        </div>
                                        ...
                                        <div class="input-group">
                                            <span class="input-group-addon">m²</span>
                                            <input class="form-control" name="assetsLuas" placeholder="Luas di Sertifikat" type="text">
                                        </div>
                                        <br/>
                                        <div class="input-group">
                                            <span class="input-group-addon">Rp</span>
                                            <input class="form-control" name="assetsHarga" placeholder="Harga" type="text">
                                        </div>
                                        <br/>
                                        <div class="form-group">
...
<script type="text/javascript">
    ...
    function showAllDoc() {
        $.ajax({
            type: 'ajax',
            url: '<?php echo base_url("member/assets/showAllDoc/".$this->uri->segment(4)) ?>',
            dataType: 'json',
            success: function(data) {
                ...
                for (i = 0; i < data.length; i++) {
                    html +='<input type="hidden" name="getDoc[]" value="' + data[i].legal_id + '" required/>' +
                            '<tr>' +
                                '<td>' + (num + i) + '</td>' +
                                '<td>' + data[i].name + '_'+ data[i].legal_id +'_'+ data[i].account_id +'_'+ data[i].assets_id + ' ' +'Belum didaftarkan</td>' +
                                '<td>' +
    ...
    function add_mass(){
            $.ajax({
                url: "<?php echo site_url('member/assets/ajax_submit')?>",
                type: "POST",
                contentType: false,
                processData: false,
                dataType: "JSON",
                success: function(data) {
                    alert("Document Masuk Boi!!!");
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    alert("Document"+' '+ $('#assestDoc').val() + ' ' + "Gk Masuk Boi");
                }
            });
    }
